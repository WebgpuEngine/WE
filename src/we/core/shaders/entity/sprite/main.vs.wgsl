...
#reflection attributes
  ...
  worldPosition= vec4f(world_matrix[attributes.instanceIndex] *sprite_matrix*vec4f(position, 1.0));
  vsOutput.worldPosition = worldPosition.xyz / worldPosition.w;
  vsOutput.position = matrix_z * MVP * world_matrix[attributes.instanceIndex] * vec4f(worldPosition.xyz, 1.0);
