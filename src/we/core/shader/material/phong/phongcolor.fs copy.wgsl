...
@fragment fn fs(fsInput : VertexShaderOutput) -> ST_GBuffer {
    ...
    if(U_lights.lightNumber >0)
    {
        for (var i : u32 = 0; i < U_lights.lightNumber; i = i + 1)
        {
            ...
            {
                computeShadow = true;
                shadow_map_index = checkPixelInShadowRangOfPointLight(fsInput.worldPosition, onelight);
            ...
            else if (onelight.kind ==2)
            {
                ...
                computeShadow = checkPixelInShadowRangOfSpotLight(fsInput.worldPosition, onelight.position, onelight.direction, onelight.angle);
            }
            ...
            if (onelight.kind ==1){
                // visibility = shadowMapVisibilityPCSS(onelight, shadow_map_index, fsInput.worldPosition, normal, 0.08); //点光源的pcss在计算block是需要适配，目前多出来了边界的黑框，目前考虑是block的uv在边界的地方越界了，需要进行特殊处理
                //下面三个在V01版本中没有问题，应该时wordPosition相关的问题
                //是因为：near的问题，near的默认值是1，没问题，0.1就出现问题，todo
                // visibility = shadowMapVisibilityPCF(onelight, shadow_map_index, fsInput.worldPosition, normal,0.08);//出现了在点光源半径3.5时，远端的实体的阴影消失问题
                // visibility = shadowMapVisibilityPCF_3x3(onelight,shadow_map_index,  fsInput.worldPosition, normal);//点光源在cube中的阴影，右下前三方向消失，其他方向存在远端消失问题
