...
fn GetTransmittance(atmosphere: AtmosphereParameters, transmittance_texture: texture_2d<f32>, r: f32, mu: f32, d: f32, ray_r_mu_intersects_ground: bool) -> vec3f {  // r,mu:起点, d:距离, 是否与地面相交
    ...
    let intersect_1: vec3f = GetTransmittanceToTopAtmosphereBoundary(atmosphere, transmittance_texture, r_d, -mu_d);
    let intersect_2: vec3f = GetTransmittanceToTopAtmosphereBoundary(atmosphere, transmittance_texture, r, -mu);
    let intersect_3: vec3f = GetTransmittanceToTopAtmosphereBoundary(atmosphere, transmittance_texture, r, mu);
    let intersect_4: vec3f = GetTransmittanceToTopAtmosphereBoundary(atmosphere, transmittance_texture, r_d, mu_d);
...
fn GetIrradianceTextureUvFromRMuS(atmosphere: AtmosphereParameters, r: f32, mu_s: f32) -> vec2f {  // r: 径向距离, mu_s: 太阳方向余弦
    ...
    return vec2f(
        ...
        1.0 - GetTextureCoordFromUnitRange(x_r, IRRADIANCE_TEXTURE_HEIGHT)
    );
}
...
fn GetSkyRadiance(atmosphere: AtmosphereParameters, transmittance_texture: texture_2d<f32>, scattering_texture: texture_3d<f32>, single_mie_scattering_texture: texture_3d<f32>, camera: vec3f, view_ray: vec3f, shadow_length: f32, sun_direction: vec3f) -> SkyRadianceResult {  // camera:相机位置, view_ray:视线方向, shadow_length:阴影长度, sun_direction:太阳方向
    ...
    let transmittance_temp = GetTransmittanceToTopAtmosphereBoundary(atmosphere, transmittance_texture, r_local, mu);
    ...
    if shadow_length == 0.0 * m {  // 无阴影：直接查询
        let result = result1; // GetCombinedScattering(atmosphere, scattering_texture, single_mie_scattering_texture, r_local, mu, mu_s, nu, ray_r_mu_intersects_ground);
        ...
        let mu_p: f32 = (r_local * mu + d) / r_p;
        let mu_s_p: f32 = (r_local * mu_s + d * nu) / r_p;
        let result = result2; // GetCombinedScattering(atmosphere, scattering_texture, single_mie_scattering_texture, r_p, mu_p, mu_s_p, nu, ray_r_mu_intersects_ground);
        scattering = result.scattering;
        single_mie_scattering = result.single_mie_scattering;
    ...
    if r > atmosphere.top_radius {  // 相机完全在大气外，无散射贡献
        return SkyRadianceResult(vec3f(0.0), vec3f(1.0));
    }
  else {
...
fn GetSkyRadianceToPoint(
    ...
    shadow_length: f32,
    sun_direction: vec3f
) -> SkyRadianceResult {  // camera:相机位置, point:目标点, shadow_length:阴影长度, sun_direction:太阳方向
    ...
    var d: f32 = length(point - camera_local);  // 到目标点距离（使用修改后的相机位置）
    let ray_r_mu_intersects_ground: bool = RayIntersectsGround(atmosphere, r_local, mu);
    ...
    let shadow_transmittance_temp: vec3f = GetTransmittance(atmosphere, transmittance_texture, r_local, mu, d, ray_r_mu_intersects_ground);
    if shadow_length > 0.0 * m {
        shadow_transmittance = shadow_transmittance_temp;
    }
...
fn vs(
    ...
    var view_ray = u_view_matrix_inverse * vec4f((u_projection_matrix_inverse * vec4f(xy, 0, 1)).xyz, 0);
...
//    init_parameter();
//     // 相同参数
//     let ray_origin = vec3f(0.0, 0.0, 0);
//     let ray_dir = normalize(vec3f(0, 0, 1));
//     let sphere_center = vec3f(0.0, 0.0, 2);
...
//         (-b - sqrt_d) / (2.0 * a),// t0：进入大气距离
//         (-b + sqrt_d) / (2.0 * a)// t1：穿出大气/碰到物体的距离
//     );
...
//     //return select( vec4f(0.0), vec4f(1.0), hit);
// }
// @fragment
...
//     // 直接使用 VS 传过来的射线
//     // ==========================
//     let ray_dir = input.view_ray;
//     let ray_origin = camera;

//     // ==========================
//     // 地球/球体相交
//     // ==========================
//     let center = earth_center;
//     let radius = 6371.0;
...
//     // 输出：击中=白色，未击中=黑色
//     // ==========================
//     return select(vec4f(0,0,0,1), vec4f(1,1,1,1), hit);
// }
...
fn fs(fs_input: VertexOutput) -> @location(0) vec4f {
    ...
    // var view_ray = u_view_matrix_inverse * u_projection_matrix_inverse * vec4f(fs_input.position.xy, 1, 1);

    let view_direction: vec3f = normalize(view_ray.xyz);  // 归一化视线方向

    let fragment_angular_size: f32 = length(fwidth(view_direction));  // 片段角尺寸（用于抗锯齿）
    ...
    let discriminant_sphere: f32 = kSphereRadius * kSphereRadius - ray_sphere_center_squared_distance;// 球体判别式，用于判断是否相交和计算交点距离

  //   let hit = discriminant_sphere > 0.0;
  //   let sqrt_d = sqrt(discriminant_sphere);

  //   var ray_length = vec2<f32>(
  //       (-p_dot_v_sphere - sqrt_d) / (2.0 * p_dot_v_sphere),// t0：进入大气距离
  //       (-p_dot_v_sphere + sqrt_d) / (2.0 * p_dot_v_sphere)// t1：穿出大气/碰到物体的距离
  //   );
  //   if hit {
  //       //交点在反方向
  //       if ray_length.x < 0.0 {
  ...
  //   }
    let safe_discriminant_sphere: f32 = select(0.0, discriminant_sphere, discriminant_sphere >= 0.0);
    let distance_to_intersection_sphere: f32 = -p_dot_v_sphere - sqrt(safe_discriminant_sphere);
    let is_sphere: bool = discriminant_sphere >= 0.0 && distance_to_intersection_sphere > 0.0;

    let point_sphere: vec3f = camera + view_direction * distance_to_intersection_sphere;
    let normal_sphere: vec3f = normalize(point_sphere - kSphereCenter);
    let safe_point_sphere: vec3f = select(camera + view_direction, point_sphere, is_sphere);
    let safe_normal_sphere: vec3f = select(vec3f(0.0, 0.0, 1.0), normal_sphere, is_sphere);

    let irradiance_result_sphere = GetSunAndSkyIrradiance_simplified(safe_point_sphere - earth_center, safe_normal_sphere, sun_direction);
    let sun_irradiance_sphere: vec3f = irradiance_result_sphere.sun_irradiance;
    let sky_irradiance_sphere: vec3f = irradiance_result_sphere.sky_irradiance;

    let shadow_length_sphere: f32 = max(0.0, min(shadow_out, distance_to_intersection_sphere) - shadow_in) * lightshaft_fadein_hack;
    let in_scatter_result_sphere = GetSkyRadianceToPoint_simplified(camera - earth_center, safe_point_sphere - earth_center, shadow_length_sphere, sun_direction);
    let in_scatter_sphere: vec3f = in_scatter_result_sphere.radiance;
    let transmittance_sphere: vec3f = in_scatter_result_sphere.transmittance;

    let ray_sphere_distance: f32 = kSphereRadius - sqrt(ray_sphere_center_squared_distance);
    let ray_sphere_angular_distance: f32 = -ray_sphere_distance / p_dot_v_sphere;
    let sphere_alpha_unclamped: f32 = min(ray_sphere_angular_distance / fragment_angular_size, 1.0);
    let sphere_radiance_unclamped: vec3f = kSphereAlbedo * (1.0 / PI) * (sun_irradiance_sphere + sky_irradiance_sphere) * transmittance_sphere + in_scatter_sphere;

    let sphere_alpha: f32 = select(0.0, sphere_alpha_unclamped, is_sphere);
    let sphere_radiance: vec3f = select(vec3f(0.0), sphere_radiance_unclamped, is_sphere);

    // return vec4f(vec3f(sphere_alpha), 1.0);
    ...
    let safe_discriminant_earth: f32 = select(0.0, discriminant_earth, discriminant_earth >= 0.0);
    let distance_to_intersection: f32 = -p_dot_v_earth - sqrt(safe_discriminant_earth);
    let is_ground: bool = discriminant_earth >= 0.0 && distance_to_intersection > 0.0;

    let point: vec3f = camera + view_direction * distance_to_intersection;
    let normal: vec3f = normalize(point - earth_center);
    let safe_point: vec3f = select(camera + view_direction, point, is_ground);
    let safe_normal: vec3f = select(vec3f(0.0, 0.0, 1.0), normal, is_ground);

    let irradiance_result = GetSunAndSkyIrradiance_simplified(safe_point - earth_center, safe_normal, sun_direction);
    ...
    let in_scatter_result = GetSkyRadianceToPoint_simplified(camera - earth_center, safe_point - earth_center, shadow_length_ground, sun_direction);
    let in_scatter: vec3f = in_scatter_result.radiance;
    let transmittance_ground: vec3f = in_scatter_result.transmittance;

    let ground_radiance_unclamped: vec3f = kGroundAlbedo * (1.0 / PI) * (sun_irradiance * GetSunVisibility(safe_point, sun_direction) +
        sky_irradiance * GetSkyVisibility(safe_point)) * transmittance_ground + in_scatter;

    let ground_alpha: f32 = select(0.0, 1.0, is_ground);
    let ground_radiance: vec3f = select(vec3f(0.0), ground_radiance_unclamped, is_ground);
