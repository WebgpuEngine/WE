...
// 透射率LUT渲染配置
override SAMPLE_COUNT: u32 = 40;            // 光线步进采样数（最小10，但能量损失开始可见）
...
@group(0) @binding(1) var transmittance_lut : texture_storage_2d<rgba16float, write>;  // 输出透射率LUT纹理
...
fn find_atmosphere_t_max_2d(t_max: ptr<function, f32>, o: vec2<f32>, d: vec2<f32>, bottom_radius: f32, top_radius: f32) -> bool {
	...
	if t_bottom < 0.0 {  // 不与地面相交
		if t_top < 0.0 { // 也不与大气顶部相交（完全在大气外）
			*t_max = 0.0;
			return false;
		} else {         // 从大气顶部进入
			*t_max = t_top;
		...
		if t_top > 0.0 { // 也与大气顶部相交，取较小值
			*t_max = min(t_top, t_bottom);
		} else {         // 在大气内部向地面传播
			*t_max = 0.0;  // 注意：这里与3D版本不同，设为0表示立即停止
		}
	}
	return true;
}
...
/**
 * 将LUT的UV坐标反变换为物理参数（观察者高度和天顶角余弦）
 * 透射率LUT的参数化方式：
 * - X轴(x_mu): 光线到达大气边界的距离归一化值
 * - Y轴(x_r): 观察者水平距离与大气厚度的比值
 * 
 * @param uv LUT采样坐标
...
fn uv_to_transmittance_lut_params(uv: vec2<f32>, atmosphere: Atmosphere) -> vec2<f32> {
	let x_mu: f32 = uv.x;  // X坐标：距离归一化值
	let x_r: f32 = uv.y;   // Y坐标：水平距离比值

	let bottom_radius_sq = atmosphere.bottom_radius * atmosphere.bottom_radius;
	let h_sq = atmosphere.top_radius * atmosphere.top_radius - bottom_radius_sq;  // 大气厚度平方
	let h: f32 = sqrt(h_sq);                                                     // 大气厚度
	let rho: f32 = h * x_r;                                                       // 观察者水平距离
	let rho_sq = rho * rho;
	let view_height: f32 = sqrt(rho_sq + bottom_radius_sq);                            // 观察者高度（从球心算起）

	// 计算光线到达大气边界的距离
	let d_min: f32 = atmosphere.top_radius - view_height;  // 天顶方向的最小距离
	let d_max: f32 = rho + h;                               // 水平方向的最大距离
	let d: f32 = d_min + x_mu * (d_max - d_min);            // 当前光线的边界距离
...
fn render_transmittance_lut(@builtin(global_invocation_id) global_id: vec3<u32>) {
	...
	let atmosphere:Atmosphere = atmosphere_buffer;
