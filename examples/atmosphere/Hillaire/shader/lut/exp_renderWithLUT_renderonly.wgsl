...
override RENDER_MOON_DISK: bool = false;
 ...
 * 核心渲染函数与资源绑定（标准混合版本）
 * 
 * renderWithLUT.wgsl 的核心渲染逻辑：
 * 1. 天空渲染：使用 sky_view_lut + 太阳/月亮圆盘
 * 2. 大气透视渲染：使用 aerial_perspective_lut 计算场景物体的雾效
 * 
 * 与 render1.wgsl 的区别：不支持双源混合，使用标量alpha作为透射率
 */
...
fn render_sky(pix: vec2<u32>) -> vec4<f32> {
	...
	// let depth = textureLoad(depth_buffer, pix, 0).r;                   // 从深度缓冲加载当前像素的深度值
	// if !is_valid_depth(depth) {                                        // 如果深度值无效（天空区域，无场景物体）
		// 天空区域：直接使用预计算的天空视图LUT
		return use_sky_view_lut(view_height, world_pos, world_dir, sun_dir, atmosphere, config);
	// }

	// // 物体区域：使用大气透视LUT计算场景物体的大气雾效
	// let depth_buffer_world_pos = uv_and_depth_to_world_pos(uv, config.inverse_projection, config.inverse_view, depth); // 将UV和深度转换为世界空间位置
	// let t_depth = length(depth_buffer_world_pos - (world_pos + atmosphere.planet_center));  // 计算从观察者到物体的距离（km）

	// var slice = aerial_perspective_depth_to_slice(t_depth);           // 将距离转换为大气透视LUT的深度切片索引
	// var weight = 1.0;                                                 // 近距离淡入权重
	// if slice < 0.5 {                                                  // 当切片索引小于0.5（非常近距离）
	// 	// 近距离时进行淡入处理，避免深度为0时的颜色突变
	// 	weight = saturate(slice * 2.0);                               // slice=0时weight=0，slice=0.5时weight=1
	// 	slice = 0.5;                                                  // 限制最小切片索引为0.5
	// }
	// let w = sqrt(slice / AP_SLICE_COUNT);                             // 平方分布映射：使近处切片采样更密集（与光线步进的二次分布类似）

	// let aerial_perspective = textureSampleLevel(aerial_perspective_lut, lut_sampler, vec3<f32>(uv, w), 0); // 查询3D大气透视LUT

	// if all(aerial_perspective.rgb == vec3<f32>())  {                  // 如果LUT值无效（全黑）
	// 	return vec4<f32>();                                          // 返回黑色
	// }

	// return weight * aerial_perspective;                               // 返回大气透视颜色（乘以近距离淡入权重）
}
