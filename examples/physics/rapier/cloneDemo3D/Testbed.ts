...
export class Testbed {
    ...
    constructor(RAPIER: RAPIER_API, builders: Builders, scene: Scene) {
        this.scene = scene;
        let backends = ["rapier"];
        this.RAPIER = RAPIER;
    ...
    //switchToDemo()-->some demo.initWorld()-->setWorld() 设置当前的物理世界 -->图形系统添加形状
    setWorld(world: RAPIER.World) {
        ...
        world.forEachCollider(async (coll) => {
            await this.graphics.addCollider(this.RAPIER, world, coll);
        });
